...
const getTzip12Metadata = async (contract, tokenId) =>
  retry(() => contract.tzip12().getTokenMetadata(tokenId), RPC_RETRY_OPTIONS);

const getTzip16MetadataView = memoizee(
  async (contract) =>
    retry(
      () =>
        contract
          ...
          .getMetadata()
          .then(({ metadata }) =>
            metadata?.views?.find((view) => view.name === "token_metadata")
          ),
      RPC_RETRY_OPTIONS
    ),
  {
...
const getContractStorageTokenMetadataUri = memoizee(
  (contract) =>
    contract.storage().then(
      (storage) => storage?.token_metadata_uri,
      (error) => {
        consola.error(
          `Caught when getting contract ${contract.address} storage. Error:`
        );
        console.error(error);
...
const getMetadataFromUri = async (contract, tokenId) => {
  ...
  const metadataUri = token_metadata_uri.replace("{tokenId}", tokenId);
  ...
  return await metadataProvider
    ...
    .then(
      ...
      (error) => {
        console.error(
          "[From URI] Caught for slug:",
          `${contract.address}_${tokenId}`,
          "Error:"
        );
        console.error(error);
...
const getTokenMetadataFromOffchainView = async (contract, tokenId, chainId) => {
  ...
  const tzip16MetadataView = await getTzip16MetadataView(contract).catch(
...
async function getTokenMetadata(contractAddress, tokenId = 0) {
  ...
  try {
    ...
    const tzip12Metadata = await getTzip12Metadata(contract, tokenId).catch(
      () => null
    );
    // TODO: Parallelize. Note, might need to wrap in retry the following call.
    //       As it is now using cached data from the previous one.
    const metadataFromUri = await getMetadataFromUri(contract, tokenId).catch(
      () => null
    );
